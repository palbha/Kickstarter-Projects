...
shinyServer(function(input, output){
  ...
  state.grp_comp <- data1 %>%
    filter(state!="undefined",state %in% c("successful", "failed",main_category=="Technology") ) %>%
    group_by( state) %>%
    mutate(grp= "complete") %>%
    summarize(count=n()) %>%
  ...
  state.grp_incomp <- data1 %>%
    filter(state!="undefined",state %in% c("canceled", "suspended","live",main_category=="Technology") ) %>%
    group_by( state) %>%
